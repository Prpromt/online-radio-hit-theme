...
<?php if(have_posts()): while(have_posts()): the_post(); $id=get_the_ID(); $thumb=orh_media_url($id,'medium'); $count=orh_artist_count($id); $label=$count==1?'трек':($count<5?'трека':'треков'); ?>
...
<div><small>АРТИСТ РАДИО ОНЛАЙН ХИТ</small><h2><?php the_title();?></h2><p><?php echo esc_html($count.' '.$label);?> в каталоге</p></div><span class="artist-arrow" aria-hidden="true">↗</span>
</a>
