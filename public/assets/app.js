(()=>{
  ...
  function setArtwork(el,i){
    ...
    const url=songs[i][2];
    el.style.backgroundImage=`linear-gradient(145deg,rgba(0,0,0,.02),rgba(255,79,163,.05)),url("${url}"),${fallbacks[i]}`;
    el.style.backgroundSize='cover';
    el.style.backgroundPosition='center';
    el.style.backgroundColor='#f1f2ef';
  }
  function artwork(i){
  ...
  $$('[data-action="repeat"]').forEach(b=>b.addEventListener('click',()=>{repeat=!repeat;b.classList.toggle('active',repeat)}));
  $('[data-action="prev"]')?.addEventListener('click',()=>setTrack(index-1,true));
  $('[data-action="next"]')?.addEventListener('click',()=>setTrack(index+1,true));
  ...
  audio.addEventListener('timeupdate',()=>{currentTime.textContent=fmt(audio.currentTime);stickyTime.textContent=fmt(audio.currentTime);if(audio.duration&&progress)progress.value=audio.currentTime/audio.duration*100});
