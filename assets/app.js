...
window.subscribeForm=function(e){e.preventDefault();var input=document.getElementById('mailEmail'),msg=document.getElementById('mailMsg');if(!input||!msg)return;fetch(ORH.ajax,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'orh_subscribe',nonce:ORH.nonce,email:input.value})}).then(function(r){return r.json();}).then(function(d){msg.textContent=(d.data&&d.data.message)||'Готово.';if(d.success)e.target.reset();}).catch(function(){msg.textContent='Не удалось отправить заявку.';});};
var radioQueueIndex=0,radioRepeat=false;
function findCurrentQueueIndex(){var q=window.ORH_QUEUE||[],a=player();if(!q.length||!a)return 0;var src=a.getAttribute('src')||a.currentSrc||'';for(var i=0;i<q.length;i++)if(q[i].url===src)return i;return 0;}
function radioLoadIndex(i,playNow){var q=window.ORH_QUEUE||[];if(!q.length)return;if(i<0)i=q.length-1;if(i>=q.length)i=0;radioQueueIndex=i;var x=q[i],a=player();if(!a)return;choose(x.title,x.artist,x.cover);a.src=x.url;a.preload='auto';var pr=document.querySelector('[data-progress]'),ct=document.querySelector('[data-current-time]'),du=document.querySelector('[data-duration]');if(pr)pr.value=0;if(ct)ct.textContent='0:00';if(du)du.textContent='0:00';activeAudio=a;try{a.load();}catch(e){}if(playNow){var p=a.play();if(p&&p.then)p.then(function(){setPlayState(true);showSticky(true);countPlay(x.id);}).catch(function(){setPlayState(false);});}}
...
document.addEventListener('DOMContentLoaded',function(){var box=document.querySelector('.player-bottom-v13');if(box)box.querySelectorAll('input[type="range"]').forEach(function(el){if(!el.matches('[data-progress],[data-volume]'))el.remove();});bindPlayer();syncInitialPlayer();tryAutoplayRadio();setTimeout(function(){tryAutoplayRadio();},1200);window.addEventListener('scroll',function(){showSticky(false);},{passive:true});showSticky(false);});
