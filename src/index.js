...
function previewHome(response) {
  return new HTMLRewriter()
    .on('head', { element(el) {
      ...
      el.append('<link rel="stylesheet" href="/assets/homepage-conversion-v1.css?v=1.0">', { html: true });
...
export default { async fetch(request, env) {
  ...
  if (url.pathname === '/artist-apply' || url.pathname === '/artist-apply/') {
    ...
    const headers = new Headers(applyResponse.headers); Object.entries(securityHeaders()).forEach(([key,value]) => headers.set(key,value)); headers.set('Cache-Control','no-store', 'max-age=0');
    return new Response(applyResponse.body, { status: applyResponse.status, statusText: applyResponse.statusText, headers });
  }
